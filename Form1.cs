...
namespace test
{
    public partial class Form1 : Form
    {
        ...
        int ThickChange = 1;
        string DeviceTestDatadirPath = @"DeviceTestData";
        /* 尋讀寫時間*/
        long FindTime, ReadTime, WriteTime, AFindTime, AReadTime, AWriteTime,ATimeCount=0;
        ...
        string[] TestResult = { "None", "None", "None", "None", "None", 
                                "None", "None", "None", "None", "None", 
                                "None", "None", "None", "None"  };
        public string[] TheWordsB2R;


        //自動測試
        int AutoSensingTimerValue = 3;
        ...
        private void Form1_KeyPress(object sender, KeyPressEventArgs e)//限制只能輸入數字
        {
            ...
            if (e.KeyChar == 13)
            {
                SAMSlotTest_Btn.Focus();
                SAMSlotTest_Btn.PerformClick();
        ...
        private void Start_BT_Click(object sender, EventArgs e)
        {
            ...
            else if (Start_BT_Forward.Focused)
            {
                ...
                Start_BT_Forward.Visible = false;
                TestBtnCount++;
        ...
        private void ComPort_CB_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (connect == false)
            {
                try
                {
                    ...
                    richTextBox1.Text += "\n請輸入讀卡機ID,並按下Enter" + Environment.NewLine;
        ...
        private void SAMSlotTest_Btn_Click(object sender, EventArgs e)
        {
            ...
            Start_BT.Text = $"{(NowCardTestType)1}" + " Card";
            ReaderStatus = 4;
            SensingTestCount = 0;
            TestBtnCount = 0;
            SAMSlotTest_BT = true;
        ...
        private void DVID_TB_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == 13)
            {
                ...
                {
                    //UI_ini();
                    richTextBox1.Text += "請置入SAM卡於底層槽後\n連接測試線" + Environment.NewLine;
                    richTextBox1.Text += "\nSAM請置於相應位置：\n" + Environment.NewLine;
                    richTextBox1.Text += "SAM5 => 一卡通\nSAM6 => ICash\nSAM7 => 一卡通\nSAM8 => ICash" + Environment.NewLine;
                    richTextBox1.Text += "\n按下SAM底層槽測試鈕進行測試" + Environment.NewLine;

                    TheWordsB2R = new String[] { "SAM", "測試線", "相應位置", "SAM底層槽測試鈕" };
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (!Directory.Exists(DeviceTestDatadirPath))
            {
                Directory.CreateDirectory(DeviceTestDatadirPath);               
            }
        ...
        private void AutoSensingTimer_Tick(object sender, EventArgs e)
        {
            richTextBox1.Text += $"\n倒數{AutoSensingTimerValue}秒自動測試";
            AutoSensingTimerValue--;            
            if (AutoSensingTimerValue == 0) 
            {
                if (SensingTestCount == 0 || SensingTestCount == StandardTimes*5)
                {
                ...
                else if (SensingTestCount == StandardTimes || SensingTestCount == StandardTimes * 6)
                {
                    Start_BT_Forward.Focus();
                    Start_BT_Forward.PerformClick();
                }
                else if (SensingTestCount == StandardTimes*2 || SensingTestCount == StandardTimes * 7)
                {
                    Start_BT_Back.Focus();
                    Start_BT_Back.PerformClick();
                }
                else if (SensingTestCount == StandardTimes*3 || SensingTestCount == StandardTimes * 8)
                {
                    Start_BT_Left.Focus();
                    Start_BT_Left.PerformClick();
                }
                else if (SensingTestCount == StandardTimes*4 || SensingTestCount == StandardTimes * 9)
                {
                    Start_BT_Right.Focus();
                    Start_BT_Right.PerformClick();
                }
               
                if (SensingTestCount == StandardTimes * 4 || SensingTestCount == StandardTimes *5 || SensingTestCount== StandardTimes*9)
                { AutoSensingTimerValue = 5;
                    
                    
                }
                else { AutoSensingTimerValue = 3;
                    
                }
                
                AutoSensingTimer.Enabled = false;
            }
            
        }

        private void AutoSensingSw_Btn_Click(object sender, EventArgs e)
        {
            if (AutoTestMode == false)
            {
                AutoTestMode = true;
                AutoSensingTimer.Enabled = true;
            }
            else 
            {
                AutoTestMode = false;
            ...
            switch (TestResult)
            {
                case 0:     //TestFail(太慢)
                    if (mode == false)
                    {
                        ...
                        AFTime_Lab.Text = (AFindTime / ATimeCount).ToString();
                        ARTime_Lab.Text = (AReadTime / ATimeCount).ToString();
                        AWTime_Lab.Text = (AWriteTime / ATimeCount).ToString();
                        ATTime_Lab.Text = ((AFindTime + AReadTime + AWriteTime) / ATimeCount).ToString();
                    }
                    break;
                case 1:
                    if (mode == false)
        ...
        private void Reader()   //卡機流程
        {
            switch (ReaderStatus)
            {
                ...
                case 0:     //尋卡
                    {
                        ...
                        if (mode || start)
                        {
                            FindTime = 0; ReadTime = 0; WriteTime = 0;
                            RecievedQ.Clear();
                            REC_count = 7;
                            REC = false;
                            ReaderStatus = 1;
                            ...
                            if (Recieved.Length == 8 || Recieved.Length == 19)
                            {
                                ...
                                switch (Recieved[5])
                                {
                                    case 0x01:  //無卡
                                        {
                                            ...
                                            TestTimesCount(0);
                                            CreatSensingTestData(Convert.ToInt32(DVID_TB.Text), "FindCardErr", SensingTestCount, ThickChange);

                                            if (SensingTestCount == SensingTestMiddleCount + StandardTimes)//將5公分的fail歸零
                                            {
                                                TestFail = 0;
                                            }
                                            ...
                                            TestBtnEnable(SensingTestCount, $"{NowCardTestType.未知}", ThickChange,StandardTimes, AutoTestMode);
                                            Array.Clear(CardID, 0, CardID.Length);
                                            ReaderStatus = 0;
                                    ...
                                    case 0x02:  //多卡重疊
                                        {
                                            TestTimesCount(0);
                                            CreatSensingTestData(Convert.ToInt32(DVID_TB.Text), "TooManyCardErr", SensingTestCount, ThickChange);
                                    ...
                                    case 0x00:  //未知
                                        {
                                            TestTimesCount(0);
                                            CreatSensingTestData(Convert.ToInt32(DVID_TB.Text), "UnknowCardErr", SensingTestCount, ThickChange);
                        ...
                        if (REC == true)
                        {
                            ...
                            else if (Recieved.Length == 16)
                            {
                                ...
                                if (!SAMExSlot_Boolen) //底層SAM
                                {
                                    ...
                                    if (Recieved[5] < 4)
                                    {
                                        richTextBox1.Text += "請拔電後將'未置入'SAM槽的部分重新檢查\n或更換下一台卡機檢測" + Environment.NewLine;
                                    ...
                                    else 
                                    {
                                        ...
                                        ReaderStatus = 4;
                                        richTextBox1.Text += "==========================" + Environment.NewLine;
                                        richTextBox1.Text += "請置入SAM卡於擴充槽後\n連接測試線" + Environment.NewLine;
                                        richTextBox1.Text += "\nSAM請置於相應位置：\n" + Environment.NewLine;
                                        richTextBox1.Text += "SAM1 => 一卡通\nSAM2 => ICash\nSAM3 => 一卡通\nSAM4 => ICash" + Environment.NewLine;
                                        richTextBox1.Text += "\n按下SAM擴充槽測試鈕進行測試" + Environment.NewLine;
                                        
                                        TheWordsB2R = new String[] { "SAM卡", "測試線", "相應位置", "SAM擴充槽測試鈕" };
                                        SetTextToRed(TheWordsB2R);
                                ...
                                else//上層SAM
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        SAMExSlotTest_Btn.Enabled = false;                                           
                                        Start_BT.Enabled = true;
                                        //Start_BT.Image =Image.FromFile("IPassCard");
                                        Start_BT.Visible = true;
                                        AutoSensingTimer.Enabled = AutoTestMode;
                                        
                                        richTextBox1.Text += "\n\n放1公分治具與一卡通(IPass)卡片於中間位置 " + Environment.NewLine;
                                        richTextBox1.Text += "\n\n按下中間位置的天線測試鈕 " + Environment.NewLine;
                                        TheWordsB2R = new String[] { "1公分", "一卡通(IPass)", "中間" };
                                        SetTextToRed(TheWordsB2R);
                                    }
                                }
                ...
                richTextBox1.Text += "\n測試結果:\n"+ "==========================" + Environment.NewLine;
                richTextBox1.Text += $"SAM底層槽: {TestResult[0]}" + Environment.NewLine;
                richTextBox1.Text += $"SAM擴充槽: {TestResult[1]}" + Environment.NewLine;
                richTextBox1.Text += $"\nIPass_1cm: {TestResult[2]}" + $"\nIPass_3cm: {TestResult[3]}" + $"\nIPass_4cm: {TestResult[4]}" + Environment.NewLine;
                richTextBox1.Text += $"\nICash_1cm: {TestResult[8]}" + $"\nICash_3cm: {TestResult[9]}" + $"\nICash_4cm: {TestResult[10]}" + Environment.NewLine;	


                        for (int i = 0; i < TestResult.Length; i++) { TestResult[i] = "None"; }
                        ...
                        if (REC == true)
                        {
                            ...
                            else if (ReaderFramewareVersion == 2|| ReaderFramewareVersion == 3)
                            {
                                ...
                                else
                                {
                                    TestTimesCount(0);
                                    richTextBox1.Text += "讀卡失敗" + Environment.NewLine;
                                    CreatSensingTestData(Convert.ToInt32(DVID_TB.Text), "IPassReadErr", SensingTestCount, ThickChange);
                                    Array.Clear(CardID, 0, CardID.Length);
                                    TestBtnEnable(SensingTestCount, $"{(NowCardTestType)1}", ThickChange, StandardTimes, AutoTestMode);
                                    if (Times == 0) { ReaderStatus = 4; }
                                    else { ReaderStatus = 0; }
                ...
                case 52:    //一卡通寫卡
                    {
                        if (REC == true)
                        {
                                ...
                                if (mode == false)
                                {
                                    ...
                                    if (!mode) 
                                    {
                                        ...
                                        if (TestBtnCount % 5 == 0 && SensingTestCount == SensingTestMiddleCount)
                                        {
                                            ...
                                            //將文字轉紅
                                            TheWordsB2R = new String[] { $"{ThickChange}公分", "一卡通" };
                                            SetTextToRed(TheWordsB2R);
                                        ...
                                        else if(SensingTestCount == SensingTestFinalCount)//FRW結果 
                                        {
                                            ...
                                            TestResult[5] = $"{(AFindTime + AReadTime + AWriteTime) / ATimeCount}";
                                            TestResult[6] = $"{HFTTime}";
                                            TestResult[7] = $"{HSTTime}";
                                            SpeedReset();
                                            start = false;
                                            TestResult[8] = "X";
                                            ...
                                            //將文字轉紅
                                            TheWordsB2R = new String[] { $"1公分", "ICash", "中間" };
                                            SetTextToRed(TheWordsB2R);
                            ...
                            else
                            {
                                TestTimesCount(0);
                                richTextBox1.Text += "交易失敗" + Environment.NewLine;
                                CreatSensingTestData(Convert.ToInt32(DVID_TB.Text), "IPassWriteErr", SensingTestCount, ThickChange);
                                TestBtnEnable(SensingTestCount, $"{(NowCardTestType)1}", ThickChange, StandardTimes, AutoTestMode);
                                Array.Clear(CardID, 0, CardID.Length);
                                if (Times == 0) { ReaderStatus = 4; }
                                else { ReaderStatus = 0; }
                ...
                case 61:    //愛金卡讀卡
                    {
                        if (REC == true)
                        {
                            ...
                            else
                            {
                                ...
                                richTextBox1.Text += "讀卡失敗" + Environment.NewLine;
                                Array.Clear(CardID, 0, CardID.Length);
                                CreatSensingTestData(Convert.ToInt32(DVID_TB.Text), "ICashReadErr", SensingTestCount, ThickChange);
                                TestBtnEnable(SensingTestCount, $"{(NowCardTestType)2}", ThickChange, StandardTimes, AutoTestMode);
                                if (Times == 0) { ReaderStatus = 4; }
                                else { ReaderStatus = 0; }
                            }
                            REC = false;
                ...
                case 62:    //愛金卡寫卡
                    {
                        if (REC == true)
                        {
                                ...
                                if (mode == false)
                                {
                                    ...
                                    if (!mode)
                                    {
                                        ...
                                        if (TestBtnCount % 5 == 0 && SensingTestCount == SensingTestMiddleCount)
                                        {
                                            ...
                                            //將文字轉紅
                                            TheWordsB2R = new String[] { $"{ThickChange}公分治具", "ICash"};
                                            SetTextToRed(TheWordsB2R);
                                        }
                                        ...
                                        if (SensingTestCount == SensingTestFinalCount)//FRW結果 
                                        {
                                            ...
                                            TestResult[11] = $"{(AFindTime + AReadTime + AWriteTime) / ATimeCount}";
                                            TestResult[12] = $"{HFTTime}";
                                            TestResult[13] = $"{HSTTime}";
                                            ReaderStatus = 5;
                                            break;
                                        }
                            ...
                            else
                            {
                                TestTimesCount(0);
                                richTextBox1.Text += "交易失敗" + Environment.NewLine;
                                CreatSensingTestData(Convert.ToInt32(DVID_TB.Text), "ICashWriteErr", SensingTestCount, ThickChange);
                                TestBtnEnable(SensingTestCount, $"{(NowCardTestType)2}", ThickChange, StandardTimes, AutoTestMode);
                                Array.Clear(CardID, 0, CardID.Length);
                                if (Times == 0) { ReaderStatus = 4; }
                                else { ReaderStatus = 0; }
        ...
        private void UI_ini()
        {
            ...
            Start_BT.Text = $"{(NowCardTestType)1}"+" Card";
            Start_BT_Forward.Text = $"{(NowCardTestType)1}" + " Card";
            Start_BT_Back.Text = $"{(NowCardTestType)1}" + " Card";
            Start_BT_Left.Text = $"{(NowCardTestType)1}" + " Card";
            Start_BT_Right.Text = $"{(NowCardTestType)1}" + " Card";
        ...
        private void TestBtnEnable(int SensingStatus,string CardType,int Thick,int TestTimes,bool AutoTestEnable) 
        {            
            ...
            {                
                ...
                richTextBox1.Text = $"將'{Thick}cm'治具與{CardType}卡片\n置於天線上方\n\n點選上方位置天線測試" + Environment.NewLine;
                Start_BT_Forward.Text = $"{CardType}" + " Card";
                Start_BT_Forward.Enabled = true;
                Start_BT_Forward.Visible = true;

                //將文字轉紅
                TheWordsB2R = new String[] { $"{Thick}cm", $"{CardType}", "上方" };
                SetTextToRed(TheWordsB2R);
            ...
            else if (SensingStatus == TestTimes*2)
            {
                ...
                Start_BT_Back.Text = $"{CardType}" + " Card";
                Start_BT_Back.Enabled = true;
                Start_BT_Back.Visible = true;

                //將文字轉紅
                TheWordsB2R = new String[] { $"{Thick}cm", $"{CardType}", "下方" };
                SetTextToRed(TheWordsB2R);
            ...
            else if (SensingStatus == TestTimes*3)
            {
                ...
                Start_BT_Left.Text = $"{CardType}" + " Card";
                Start_BT_Left.Enabled = true;
                Start_BT_Left.Visible = true;

                //將文字轉紅
                TheWordsB2R = new String[] { $"{Thick}cm", $"{CardType}", "左方" };
                SetTextToRed(TheWordsB2R);
            }
            else if (SensingStatus == TestTimes*4)
            {
                richTextBox1.Text = $"將'{Thick}cm'治具與{CardType}卡片置於天線右方\n\n點選右方位置天線測試" + Environment.NewLine;
                Start_BT_Right.Text = $"{CardType}" + " Card";
                Start_BT_Right.Enabled = true;
                Start_BT_Right.Visible = true;

                //將文字轉紅
                TheWordsB2R = new String[] { $"{Thick}cm", $"{CardType}", "右方" };
                SetTextToRed(TheWordsB2R);
            }
            else if (SensingStatus == TestTimes*5)
            {
                Start_BT.Text = $"{CardType}" + " Card";
                Start_BT.Enabled = true;
                Start_BT.Visible = true;
            }
            else if (SensingStatus == TestTimes*6)
            {
                if (CardType == $"{(NowCardTestType)1}")
                {
                    if (TestFail == 0)
                    {
                        TestResult[4] = "O";
                ...
                else if (CardType == $"{(NowCardTestType)2}")
                {
                    if (TestFail == 0)
                    {
                        TestResult[10] = "O";
                    ...
                    ThickChange = 3;
                Thick = ThickChange;
                richTextBox1.Text = $"將'{Thick}cm'治具與{CardType}卡片置於天線上方\n\n點選上方位置天線測試" + Environment.NewLine;
                Start_BT_Forward.Text = $"{CardType}" + " Card";
                Start_BT_Forward.Enabled = true;
                Start_BT_Forward.Visible = true;
            ...
            {
                richTextBox1.Text = $"將'{Thick}cm'治具與{CardType}卡片置於天線下方\n\n點選下方位置天線測試" + Environment.NewLine;
                Start_BT_Back.Text = $"{CardType}" + " Card";
                Start_BT_Back.Enabled = true;
                Start_BT_Back.Visible = true;
        ...
        {
            int unixTime = Convert.ToInt32(DateTimeOffset.Now.ToUnixTimeSeconds());
            String TestDataFileName = "DeviceTestData\\D" + $"{DeviceID}" + "_TestData.csv", now;            
        ...
        private void CreatTestResultData(int DeviceID, string[] TestResultArray)
        {
            ...
            if (!File.Exists(TestDataFileName))
            {
                using (StreamWriter sw = new StreamWriter(TestDataFileName, true, System.Text.Encoding.Default))
                {
                    ...
                    sw.WriteLine(
                        "DeviceID" + "," + "SAMSlot_Bottom," + "SAMSlot_Bottom,"+"IPass_1cm_FRWResult," + "IPass_3cm_FRWResult," + "IPass_4cm_FRWResult," + "IPass平均," + "IPass最快," + "IPass最慢,"
                        + "ICash_1cm_FRWResult," + "ICash_3cm_FRWResult," + "ICash_4cm_FRWResult," + "ICash平均," + "ICash最快," + "ICash最慢,"
                        );
                    sw.WriteLine(
                          "D" + $"{DeviceID}," + $"{TestResultArray[0]}," + $"{TestResultArray[1]}," + $"{TestResultArray[2]}," + $"{TestResultArray[3]}," + $"{TestResultArray[4]}," + $"{TestResultArray[5]},"+
                          $"{TestResultArray[6]}," + $"{TestResultArray[7]}," + $"{TestResultArray[8]}," + $"{TestResultArray[9]}," + $"{TestResultArray[10]}," + $"{TestResultArray[11]}," + $"{TestResultArray[12]},"
                          + $"{TestResultArray[13]},"
        ...
        private void SetTextToRed(String[] Keyword) 
        {
            for (int i = 0; i < Keyword.Length; i++) 
            {
                int index = richTextBox1.Text.IndexOf(Keyword[i]);
